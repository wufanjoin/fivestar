...
public static class TraverseDirectory
{
	...
	public static IEnumerable<string> Do(string path)
	{
		...
		while (traversal.Count > 0)
		{
			...
			//LogMgr.Ins.LogInfo("in " + currentStack.TopFolder);
			...
			for (long n = currentStack.ChildIdxToGoOnReturn, len = allInCurrentFolder.Length; n < len; ++n)
			{
				...
				//LogMgr.Ins.LogInfo("for loop: [" + n + "] " + allInCurrentFolder[n]);
				...
				{
					//LogMgr.Ins.LogInfo("is folder: " + allInCurrentFolder[n]);
					...
					//LogMgr.Ins.LogInfo("pushed " + allInCurrentFolder[n]);
			...
			if (!toGoDeeper)
			{
				//LogMgr.Ins.LogInfo("popped " + currentStack.TopFolder);
				traversal.Pop();
			}
		}
	}
