...
namespace ETModel
{
	...
	{
	    public static ResourcesComponent Ins;
        public static AssetBundleManifest AssetBundleManifestObject { get; set; }
	    public static readonly Dictionary<string, string> bundlesNameDictionary = new Dictionary<string, string>();
	    ...
	    public static void SetAssetBundleManifestObject(AssetBundleManifest assetBundleManifest)
	    {
	        AssetBundleManifestObject = assetBundleManifest;

            string[] allAssetBundles = AssetBundleManifestObject.GetAllAssetBundles();
	        RecordAssbundlsName(allAssetBundles);

	    }

	    public static void RecordAssbundlsName(string[] allBundleName)
	    {
	        foreach (var assetBundle in allBundleName)
	        {
	            string[] nameSplit = assetBundle.Split('/');
	            if (nameSplit.Length > 1)
	            {
	                if (!bundlesNameDictionary.ContainsKey(nameSplit[1]))
	                {
	                    bundlesNameDictionary.Add(nameSplit[1], nameSplit[0]);
	                }
	                else
	                {
	                    Log.Error("有名字相同的Bundle即使不在同一文件夹也不行名字:" + assetBundle);
	                }
	            }
	        }
        }
	    ...
	    public UnityEngine.Object GetAsset(string bundleName, string prefab)
	    {
	        ...
	        return BundleNameGetAsset(bundleName, prefab);
	    }
        public UnityEngine.Object BundleNameGetAsset(string bundleName, string prefab)
		{
		    if (!resourceCache.ContainsKey(bundleName))
		    {
		        this.AwayPrefixLoadBundle(bundleName);//如果ab没有 加载 就先加载
		    }
			...
			{
				//throw new Exception($"not found asset: {bundleName} {prefab}");
                Log.Debug($"找不到资源: 包名:{bundleName} 资源名:{prefab}");
			}
