...
public class Reporter : MonoBehaviour
{
	...
	void drawToolBar()
	{
        ...
        if (GUILayout.Button(infoContent, barStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
        {
            GameObject go=Resources.Load<GameObject>("Prefabs/UI/Global/NetWorkSiteView");
            GameObject.Instantiate(go, GameObject.Find("Main/Canvas3").transform);
            show = false;
...
#else
		currentScene = Application.loadedLevelName;
		LogMgr.Ins.LogInfo("Scene " + Application.loadedLevelName + " is loaded");
#endif
	}
